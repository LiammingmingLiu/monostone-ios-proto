...
```
iOS = 完整独立产品
      录音 / 产物 / 日程管理 / 检索 / 投递 / 分享 / Agent（增强层入口）/ 记忆 / Daily Digest
...
| 层级          | 能力                                                                         | 说明                                   |
| ----------- | -------------------------------------------------------------------------- | ------------------------------------ |
| 🔵 **Core** | 录音+转录、总结+结构化、Insight、Daily Digest、**日程管理**、Timeline、Project、Memory、搜索、文件导入 | 没有就不是 Monostone                      |
| 🟡 **增强层**  | Agent IM、投递（Deliver）、Smart Follow-up、Context Connection                    | 有了更好，没有产品也成立                         |
...
- 插件市场 & 第三方插件开放 → V1.5
- Agent IM 是增强层，不是核心——用户不跟 Agent 聊天也能完成核心流程
...
| 3 | **Memory** | 记忆展示 & 编辑 |
| 4 | **Me** | Profile + Settings + Agent IM 入口 |

Agent IM 入口（不再是独立 Tab）：
- Me Tab 顶部 "💬 Ask Mono" 按钮 → 全屏对话
- 产物详情页底部 "💬 Ask about this" → 带 context 的对话
- 全局浮动按钮（可选，类似 ChatGPT 圆形按钮）

Settings 入口：**Me Tab**（非 Timeline 左上角头像）
...
**Agent IM 不再是独立 Tab，而是增强层入口。** 用户不跟 Agent 聊天也能完成核心流程（录音 → 产物 → 日程 → Digest）。Agent 是"想深度处理时打开"的工具。

**入口：**
- Me Tab 顶部 "💬 Ask Mono" 按钮 → 全屏对话（Push）
- 产物详情页底部 "💬 Ask about this" → 带 context 的全屏对话
- 全局浮动按钮（可选，类似 ChatGPT 圆形按钮）
...
- 可手动引用某条 Timeline 条目作为 context（"基于这条录音..."）

**能力范围：**
- 基于录音内容生成邮件草稿、会议简报、follow-up
- 回答关于用户历史内容的问题（"上周和 Eric 聊了什么"）
- 创建日历事件、提醒
- 文本处理（翻译、改写、摘要）
- 投递到指定目的地
- 执行已安装的 Agent 插件能力
- 基于 plan + credits，用户可解锁更强能力

**不做（默认 plan）：**
- 复杂 coding 任务（高级 plan 可解锁）
- 大量文档批量处理（高级 plan 可解锁）
...
### 1.8 Me Tab & Settings（Tab 4）

入口：**底部 Tab Bar 第 4 个 Tab "Me"**

**Me Tab 顶部：**
- 💬 "Ask Mono" 按钮 → Agent IM 全屏对话
- Profile 卡片（头像、用户名、Plan、Credits）
...
- [ ] 分享（Share Sheet）
- [ ] Agent IM（Me Tab 入口 + 产物详情入口 + 全局浮动按钮）
- [ ] Memory 展示 & 编辑
- [ ] Me Tab（Profile + Settings + Integrations）
