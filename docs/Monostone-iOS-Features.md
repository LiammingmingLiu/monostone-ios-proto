...
│   └── → BYOK 配置页
│
├── Profile & Settings（Timeline 左上角头像入口 → Push 页面）
│   ├── Profile 卡片（头像 / Plan / Credits）
│   ├── → 戒指管理页
│   │   └── → 配对引导页
...
**布局：**
- 顶部：左侧用户头像（→ Profile & Settings 页面，Push 进入）、中间 "Timeline" 标题、右侧戒指连接状态图标
...
### 2.5 Agent Tab（Tab 4 — 核心功能）

**路径：** Tab 4 根页面

**Agent 是 Monostone 的核心功能之一**，类似 OpenClaw 的 Agent 体验。用户在这里与 Mono 对话、下达指令、管理 AI 能力。Agent 既是对话界面，也是 BYOK / Max Plan 的变现入口。

**布局：**

```
┌─────────────────────────────┐
│ [Scope: 全局 ▾]    [+] [⚙️] │  ← Scope 选择 + 新对话 + Agent 设置
├─────────────────────────────┤
│                              │
│   💬 对话区域                 │
│   （气泡流，支持丰富内容）     │
...
**顶部操作栏：**
- **Scope 选择器**：默认"全局" / 可选某个 active Project。选中 Project scope 后，Agent 自动加载该 Project Memory 作为 context
- **新对话按钮（+）**：开始新对话，保留历史
- **Agent 设置按钮（⚙️）**→ Push 到 Agent 设置页：
  - **Model 选择**：显示当前使用的模型（如 "Mono Default" / "GPT-4o" / "Claude Sonnet"）
  - **BYOK 配置**：用户可输入自己的 API Key（OpenAI / Anthropic / 其他），使用自带额度
  - **Plan 信息**：当前 Plan（Free / Pro / Max）+ 剩余 Credits + 升级入口
  - **能力说明**：当前 Plan 解锁的功能列表

**对话气泡类型：**
- 纯文本消息（Markdown 渲染）
- Agent 执行结果卡片（内联展示：标题 + 预览 + "查看详情"按钮 → 跳转产物详情页）
- Agent 请求确认卡片（"是否发送这封邮件？" + 确认/取消按钮）
- Agent 思考中状态（typing indicator + 模型名称标识）
- **Tool 调用卡片**（新增）：展示 Agent 正在调用的工具（如"正在查询日历..."、"正在创建 Notion 页面..."）

**底部输入栏：**
- 📎 附件按钮：选择 Timeline 条目 / 文件 / 图片作为 context
- 文字输入框
- 🎙️ 语音输入：长按录音，松开发送（同时支持戒指录入）
- ➤ 发送按钮

**特殊交互：**
- 长按消息 → 复制 / 引用 / 重新生成
- 引用 Timeline 条目：通过 📎 附件按钮选择
- 对话历史持久化，可上滑加载更早的对话
- 左滑消息 → 引用回复

**BYOK 模式说明：**
- 用户自带 API Key 时，Agent 直接使用用户的模型额度
- Monostone 不收取 AI 调用费用，只收取平台订阅费
- 支持的 Provider：OpenAI / Anthropic / Google / 自定义 OpenAI-compatible endpoint
- BYOK 用户可使用任意模型（包括 o1-pro、Opus 等高端模型），不受 Plan 限制

**Plan 层级对 Agent 能力的影响：**
...
**产物详情页入口保留：**
- 产物详情页底部仍有 "💬 Ask about this" 按钮 → 跳转 Agent Tab，自动携带该产物作为 context
...
### 2.8 Profile & Settings（从 Timeline 左上角头像进入）

**路径：** Timeline 左上角头像 → Push 进入

**不再是独立 Tab。** Profile & Settings 降级为从 Timeline 顶部左侧头像进入的设置页面，腾出 Tab 4 给 Agent。
...
| 13 | 记忆总览页 | Tab 3 根页面 |
| 14 | Agent 主页（对话 + 能力面板） | Tab 4 根页面 |
| 15 | Agent 设置页（Model / BYOK / Plan） | Push 页面 |
| 16 | Profile & Settings 页 | Push 页面（Timeline 左上角头像入口） |
| 17 | 戒指管理页 | Push 页面 |
| 18 | 配对引导页（Settings 入口） | Modal 流程 |
| 19 | Integrations 页（日历连接 + Destination） | Push 页面 |
| 20 | Destination 详情页 | Push 页面 |
| 21 | 账户 & 订阅页 | Push 页面 |
| 22 | 升级 Plan 页 | Modal 页面 |
| 23 | 隐私 & 数据页 | Push 页面 |
| 24 | 高级 / 诊断页 | Push 页面 |
| 25 | 投递选择页 | Bottom Sheet |
| — | 录音中状态条 | 全局覆盖层 |
| — | 戒指断连提示条 | 全局覆盖层 |

**共 25 个页面 + 2 个全局覆盖层。**

> v5 变更：Agent 升级为 Tab 4 核心功能（类 OpenClaw 体验，BYOK + Max Plan 变现入口）。原 Me Tab 降级为 Timeline 左上角 Profile 入口。全局 Agent 浮动按钮移除（Agent 已有独立 Tab，不需要浮动入口）。
